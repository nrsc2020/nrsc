...
pub fn prepare_catchup_chain(catchup_req: CatchupReq) -> Result<CatchupChain> {
    ...
    loop {
        let joint = NRSC_CACHE.get_joint(&last_ball_unit)?.read()?;
...
pub fn process_catchup_chain(catchup_chain: CatchupChain) -> Result<Vec<String>> {
    ...
    || -> Result<()> {
        ...
        let joint = match NRSC_CACHE.get_ball_unit_hash(&chain_balls[len - 1])? {
            Some(unit) => NRSC_CACHE.get_joint(&unit)?.read()?,
        ...
        let second_ball_joint = match NRSC_CACHE.get_ball_unit_hash(&chain_balls[len - 2])? {
            None => return Ok(()),
            Some(unit) => NRSC_CACHE.get_joint(&unit)?.read()?,
        };
...
pub fn prepare_hash_tree(hash_tree_req: HashTreeReq) -> Result<Vec<BallProps>> {
    ...
    let from_joint = match NRSC_CACHE.get_ball_unit_hash(&from_ball)? {
        Some(unit) => NRSC_CACHE.get_joint(&unit)?.read()?,
        None => bail!("from_ball {} is not known", from_ball),
    };

    let to_joint = match NRSC_CACHE.get_ball_unit_hash(&to_ball)? {
        Some(unit) => NRSC_CACHE.get_joint(&unit)?.read()?,
    ...
    while from_mci <= to_mci {
        let joints = NRSC_CACHE.get_joints_by_mci(from_mci)?;
        for joint in joints {
...
pub fn process_hash_tree(balls: &[BallProps]) -> Result<()> {
    use crate::nrsc_object_base::object_hash;

    for ball_prop in balls {
        // skip the already known ones
        if NRSC_CACHE.get_joint(&ball_prop.unit).is_ok() {
            continue;
        }
        ...
        NRSC_CACHE.add_hash_tree_ball(ball, ball_prop.unit.clone());
    }
