...
pub fn prepare_witness_proof(
    ...
    //     return Err(NRSCError::WitnessChanged.into());
    // }
...
fn process_witness_change_and_definition(
    ...
    for address in MY_WITNESSES.iter() {
        if let Some((_, definition)) = NRSC_CACHE.get_definition(address) {
    ...
    for joint in witness_change_and_definition {
        ...
        if from_current {
            ...
            if let Ok(joint) = NRSC_CACHE.get_joint(unit_hash) {
