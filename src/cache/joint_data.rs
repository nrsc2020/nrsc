    ...
    pub fn get_max_stable_unit(&self) -> Result<RcuReader<JointData>> {
        if self.is_min_wl_increased() {
            if !self.max_stable_unit.is_completed() {
                let joint = NRSC_CACHE.get_joint(&self.unit.unit)?.read()?;
...
impl JointData {
    ...
    fn calc_best_parent(&self) -> Result<()> {
        ...
        if let Some(best_parent) = find_best_joint(self.parents.iter())? {
            let best_parent = NRSC_CACHE.get_joint(&best_parent.unit.unit)?;
    ...
    pub fn get_last_ball_joint(&self) -> Result<RcuReader<JointData>> {
        ...
        match self.unit.last_ball_unit {
            Some(ref unit) => NRSC_CACHE.get_joint(unit)?.read(),
            None => NRSC_CACHE.get_joint(&::spec::GENESIS_UNIT)?.read(),
        }
    }
...
impl LoadFromKv<String> for JointData {
    fn load_from_kv<T: ::std::borrow::Borrow<String>>(key: &T) -> Result<Self> {
        ...
        let parents = joint
            ...
            .parent_units
            .iter()
            .map(|key| NRSC_CACHE.get_joint_or_none(key))
            .collect();
        ...
        best_parent.append(NRSC_CACHE.get_joint_or_none(&props.best_parent_unit));
