...
use nrsc::business::BUSINESS_CACHE;
use nrsc::cache::{CachedJoint, JointData, NRSC_CACHE};
use nrsc::error::Result;
use nrsc::joint::JointSequence;
use nrsc::joint::Level;
use nrsc::my_witness::MY_WITNESSES;
use nrsc::wallet_info::MY_WALLET;
use nrsc_wallet_base::Base64KeyExt;
...
lazy_static! {
    ...
    static ref SELF_LEVEL: AtomicIsize = AtomicIsize::new(1 - nrsc::config::MAJORITY_OF_WITNESSES as isize);
...
fn is_need_witnessing() -> Result<(bool)> {
    let free_joints = NRSC_CACHE.get_all_free_joints();
    ...
    let best_joint = nrsc::main_chain::find_best_joint(free_joints.iter())?
...
fn check_self_level(free_joints: &[CachedJoint]) -> Result<(bool)> {
    ...
    for unit in free_joints {
        ...
        if level.value() as isize - self_level >= nrsc::config::MAJORITY_OF_WITNESSES as isize - 2 {
            return Ok(true);
        }
    }
...
fn is_relative_stable(mut best_free_parent: RcuReader<JointData>) -> Result<(bool, bool)> {
    ...
    while !(best_free_parent.is_stable() || best_free_parent.unit.is_genesis_unit()) {
        ...
        if diff_witnesses.len() >= nrsc::config::MAJORITY_OF_WITNESSES - 1 {
            break;
        }
...
fn is_need_witness_normal_joint(
    ...
    if let Some(joint) = get_unstable_latest_normal_joint(free_joints)? {
        if let Some(joint) = find_best_include_mc_joint(best_joint.clone(), joint)? {
            ...
            if SELF_TIME.load(Ordering::Relaxed) + 10_000 > nrsc::time::now() as usize
                && nrsc::main_chain::is_stable_to_joint(&joint, &best_joint)?
            {
                return Ok(false);
            }
        }
...
fn compose_and_normalize() -> Result<()> {
    let nrsc::composer::ParentsAndLastBall {
        parents,
        last_ball,
        last_ball_unit,
    } = nrsc::composer::pick_parents_and_last_ball(&MY_WALLET._00_address)?;
    ...
    let light_props = nrsc::light::LightProps {
        last_ball,
        last_ball_unit,
        parent_units: parents,
        witness_list_unit: nrsc::spec::GENESIS_UNIT.to_string(),
        has_definition: NRSC_CACHE.get_definition(&MY_WALLET._00_address).is_some(),
    };

    let compose_info = nrsc::composer::ComposeInfo {
        paid_address: MY_WALLET._00_address.clone(),
        change_address: MY_WALLET._00_address.clone(),
        outputs: Vec::new(),
        inputs: nrsc::light::InputsResponse { inputs, amount },
    ...
    // if nrsc::config::get_need_post_timestamp() {
    //     let time_stamp = TimeStamp {
    //         timestamp: nrsc::time::now() / 1_000,
    //     };
    //     let data_feed_msg = nrsc::spec::Message {
    //         app: "data_feed".to_string(),
    //         payload_location: "inline".to_string(),
    //         payload_hash: object_hash::get_base64_hash(&time_stamp)?,
    //         payload: Some(nrsc::spec::Payload::Other(serde_json::to_value(
    //             time_stamp,
    //         )?)),
    //         payload_uri: None,
    ...
    let joint = nrsc::composer::compose_joint(compose_info, &*MY_WALLET)?;

    let cached_joint = NRSC_CACHE.add_new_joint(joint, None)?;

    let joint_data = cached_joint.read()?;
    joint_data.set_is_post(true);
    nrsc::validation::validate_ready_joint(cached_joint)?;
    let sequence = joint_data.get_sequence();
    if sequence != JointSequence::Good {
        // purge the bad composed joint
        NRSC_CACHE.purge_free_joint(&joint_data.unit.unit)?;
        bail!(
    ...
    SELF_TIME.store(nrsc::time::now() as usize, Ordering::Relaxed);
    ...
    if let Some(ws) = nrsc::network::hub::WSS.get_next_peer() {
