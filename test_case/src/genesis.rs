...
use nrsc::{config, joint::Joint, spec::*};
use nrsc_object_base::object_hash;
use nrsc_wallet_base::Base64KeyExt;

pub struct NRSCInitInfo {
    pub witnesses: Vec<WalletInfo>,
    pub nrsc_org: WalletInfo,
}

pub fn gen_all_wallets(witness_counts: u32) -> Result<NRSCInitInfo> {
    ...
    Ok(NRSCInitInfo {
        witnesses: witnesses,
        nrsc_org: WalletInfo::from_mnemonic("")?,
    })
}

// generate genesis unit according to the params
// return: (joint, fundation's token)
pub fn gen_genesis_joint(wallets: &NRSCInitInfo, total: u64, msg: &str) -> Result<(Joint, u64)> {
    ...
    let mut unit = Unit {
        messages: vec![nrsc::composer::create_text_message(&String::from(msg))?],
        earned_headers_commission_recipients: vec![HeaderCommissionShare {
            // give the header commission to nrsc_org
            address: wallets.nrsc_org._00_address.clone(),
    ...
    let foundation_amount;
    {
        ...
        if let Some(Payload::Payment(ref mut x)) = payment_message.payload {
            for output in x.outputs.iter_mut() {
                if output.address == wallets.nrsc_org._00_address {
    ...
    for author in &mut unit.authors {
        ...
        {
            let signature = nrsc_wallet_base::sign(&unit_hash, &index._00_address_prvk)?;
            author.authentifiers.insert("r".to_string(), signature);
        }
    }

    unit.timestamp = Some(::nrsc::time::now() / 1000);
...
pub fn gen_first_payment(
    ...
    for author in &mut unit.authors {
        let signature = nrsc_wallet_base::sign(&unit_hash, &paying_wallet._00_address_prvk)?;
        author.authentifiers.insert("r".to_string(), signature);
    }
