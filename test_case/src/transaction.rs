...
pub fn send_payment(
    ...
    let mut compose_info = nrsc::composer::ComposeInfo {
    ...
    let normal_joint = nrsc::composer::compose_joint(compose_info.clone(), wallet_info)?;
...
pub fn distribute_token(
    ...
    for _ in 0..count {
        for chunk in address_amount.chunks(nrsc::config::MAX_OUTPUTS_PER_PAYMENT_MESSAGE - 1) {
