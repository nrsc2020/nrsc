...
use nrsc::cache::NRSC_CACHE;
use nrsc::error::Result;
use nrsc::joint::{Joint, JointSequence};
use nrsc::network::wallet::WalletConn;
use nrsc::statistics::{LastConnStat, StatsPerPeriod};
use nrsc::try_go;
use nrsc::validation;
use nrsc::wallet_info::{WalletInfo, MY_WALLET};
...
fn connect_to_remote(peers: &[String]) -> Result<Arc<WalletConn>> {
    for peer in peers {
        match nrsc::network::wallet::create_outbound_conn(&peer) {
            Err(e) => {
...
fn send_payment(
    ...
    let text_message = match text {
        Some(msg) => Some(nrsc::composer::create_text_message(msg)?),
        None => None,
    };
    ...
    let outputs = address_amount
        .iter()
        .map(|(address, amount)| nrsc::spec::Output {
    ...
    let inputs: nrsc::light::InputsResponse = ws.get_inputs_from_hub(
    ...
    let joint = nrsc::composer::compose_joint(compose_info, wallet_info)?;
    ...
    println!(
        "DATE  : {}",
        Local
            .timestamp_millis(nrsc::time::now() as i64)
            .naive_local()
    );
...
fn verify_joints(joints: Vec<Joint>, last_mci: usize) -> Result<()> {
    ...
    while !sem.wait_timeout(Duration::from_secs(1)) {
        println!("current mci={:?}", nrsc::main_chain::get_last_stable_mci());
    }

    let dur = now.elapsed();
    println!("\ntotal mci={:?}", nrsc::main_chain::get_last_stable_mci());
...
fn register_event_handlers(last_mci: usize, sem: Arc<Semphore>) {
    use nrsc::main_chain::MciStableEvent;
    use nrsc::utils::event::Event;
...
fn main() -> Result<()> {
    ...
    if let Some(init_arg) = m.subcommand_matches("init") {
        if let Some(mnemonic) = init_arg.value_of("MNEMONIC") {
            nrsc::config::update_mnemonic(mnemonic)?;
        }
        // create settings
        let settings = nrsc::config::get_settings();
        settings.show_config();
        // every init would remove the local database
        // ::std::fs::remove_file(nrsc::config::get_database_path(true)).ok();
        return Ok(());
    }
